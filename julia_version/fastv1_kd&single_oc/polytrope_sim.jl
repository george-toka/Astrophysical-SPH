module polytropeSim	
	...
	function getAcc(pos::Matrix{Float64}, vel::Matrix{Float64}, m::Float64, K::Vector{Float64}, gamma::Float64, G::Float64, theta::Float64, alpha::Float64, beta::Float64, Kh::Int64)
		#=
		...
		ax, ay, az, rho, dWx, dWy, dWz, h, cs, mu, indices, vij_x, vij_y, vij_z, Pi_ij = HJL.hydrodynamics(pos, vel, m, K, gamma, alpha, beta, Kh)
		...
		return acc, rho, dWx, dWy, dWz, h, cs, mu, indices, vij_x, vij_y, vij_z, Pi_ij, PHI
	end
	...
	function run_simulation(ic_type::String, snapID::Int, snapInterval::Int, keepSnaps::Bool, showPlots::Bool)
		...
		# Plotting parameters 
		intervalCounter = snapInterval    # Counter that resets everytime we snapshot
		...
		plotN = 10000
		rr = zeros(plotN, 3)
		rlin = LinRange(0, 1.5*R, plotN)  # scale sample points to domain scale
		...
		while t < tEnd 
			...
			acc, rho, dWx, dWy, dWz, h, cs, mu, indices, vij_x, vij_y, vij_z, Pi_ij, PHI = getAcc(pos, vel, m, K, gamma, G, theta, alpha, beta, Kh)
			...
			HJL.evolve_K!(K, m, indices, Pi_ij, v_dot_dW, gamma ,rho, dt)
			acc, rho, dWx, dWy, dWz, h, cs, mu, indices, vij_x, vij_y, vij_z, Pi_ij, PHI = getAcc(pos_half, vel_half, m, K, gamma, G, theta, alpha, beta, Kh)
			
			# Evolve K another half step - Go to whole step
			HJL.evolve_K!(K, m, indices, Pi_ij, vij_x, vij_y, vij_z, dWx, dWy, dWz, gamma, rho, dt)
			...
			if keepSnaps * intervalCounter == snapInterval || t >= tEnd
				...
				if showPlots || t >= tEnd
					...
					# === Figure 1 ===
					# Subplot 1: Star scatter - Normalise values to plot, based on initial radius
					...
					Mmap.sync!(stats_arr)
					SnapshotRW.write_snapshot(string(iterID), ic_type, pos, vel; K, constants,rlin, rho_radial, fig1, fig2)

				else
					# Ensure write back / Bin statistics and graphs
					Mmap.sync!(stats_arr)
					SnapshotRW.write_snapshot(string(iterID), ic_type, pos, vel; K, constants,rlin, rho_radial)
				end
