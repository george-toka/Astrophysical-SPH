...
function write_snapshot(snapID::String,
    ...
    df = DataFrame(
        ...
        K = Vector{Union{Missing, Float64}}(missing, N),
    ...
    # Add K if provided
    if K !== nothing
        df.K = Vector{Union{Missing, Float64}}(K)
    end
    ...
    CSV.write("./snapshots/" *ic_type * "/bin/" * snapID * "snap.csv", df)

    # Save figures if provided
    if fig1 !== nothing 
        save("./snapshots/" *ic_type * "/graphs/" * snapID * "_star.png", fig1)
    end
    if fig2 !== nothing 
        save("./snapshots/" *ic_type * "/graphs/" * snapID * "_stats.png", fig2)
    end
...
function read_snapshot(filename::String)
    ...
    K   = hasproperty(particle_rows, :K) ? particle_rows.K : nothing

    # Parse rlin row
    rlin_row = filter(row -> row[:type] == "rlin", eachrow(df))
    rlin = (length(rlin_row) == 1) ? parse.(Float64, split(rlin_row[1][:rlin], ';')) : Float64[]

    # Parse rho_radial row
    rho_radial_row = filter(row -> row[:type] == "rho_radial", eachrow(df))
    rho_radial = (length(rho_radial_row) == 1) ? parse.(Float64, split(rho_radial_row[1][:rho_radial], ';')) : Float64[]
    ...
    if length(constants_row) == 1
        ...
        for pair in split(const_str, ';')
            k, v = split(pair, '=')
            constants[k] = occursin(r"[eE.]", v) ? parse(Float64, v) : parse(Int, v)
        end
    end
